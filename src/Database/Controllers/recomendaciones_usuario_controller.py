...
def asignar_infimas_a_usuarios(db: Session, max_por_usuario: int = 15):
    ...
    subquery_infimas_asignadas = (
        db.query(RecomendacionesUsuario.id_infima).distinct().subquery()
    )

    # identifiamos infimas disponibles y que no han sido asignadas
    infimas_disponibles = (
        db.query(Infima)
        .filter(not_(Infima.id_infima.in_(subquery_infimas_asignadas)))
        .all()
    )
    ...
    for usuario in usuarios:
        ...
        while asignadas_usuario < max_por_usuario and idx_infima < len(
            infimas_disponibles
        ):
            # crea objetos
            asignaciones.append(
    ...
    return {
        ...
        "detalle": [
            {"usuario_id": a.usuario_id, "id_infima": a.id_infima} for a in asignaciones
        ],
    }
