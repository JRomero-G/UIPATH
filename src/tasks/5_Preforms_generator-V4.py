...
Flujo general (resumen de las 9 etapas del requerimiento):
  ...
  4) Con Gemini (búsqueda web con grounding) + Gemini 3.1 Pro (endpoint global)
...
#               "google-genai>=1.51.0" python-docx openpyxl \
#               pillow requests
#   (Ya NO se necesita "anthropic[vertex]": el motor de selección ahora es Gemini.
#    Se recomienda google-genai >= 1.51.0 para usar gemini-3.1-pro-preview.)
import requests
...
CONFIG_LOCAL = {
    ...
    # --- Regiones/endpoints de Vertex AI (opcional; cámbialas si tu proyecto usa otras) -
    "VERTEX_LOCATION_GEMINI":    "us-central1",   # región del proyecto (Iowa): análisis + grounding
    "VERTEX_LOCATION_SELECCION": "global",        # endpoint del motor de selección (gemini-3.1-pro-preview)
}
...
AI_MODEL_SELECCION          = "gemini-3.1-pro-preview"  # razonamiento/selección de proveedor (reemplaza a "Sonnet 4.6")
AI_MODEL_SELECCION_FALLBACK = "gemini-2.5-pro"          # alternativa GA si el proyecto aún no tiene habilitado 3.1 Pro

# Regiones/endpoints de Vertex AI (ajustar según disponibilidad del proyecto).
#   • us-central1 : región del proyecto (Iowa). Sirve gemini-2.5-pro para el análisis
#                   multimodal de documentos y para la búsqueda con grounding.
#   • global      : endpoint donde se sirve gemini-3.1-pro-preview. ESTE modelo NO existe
#                   en us-central1 (una llamada regional devolvería 404), por lo que la
#                   selección usa un cliente aparte apuntando al endpoint global. Como
#                   beneficio extra, el endpoint global reduce los errores 429 (cuota) y
#                   NO cambia la región del resto de los recursos del proyecto.
VERTEX_LOCATION_GEMINI    = _g("VERTEX_LOCATION_GEMINI",    "us-central1")
VERTEX_LOCATION_SELECCION = _g("VERTEX_LOCATION_SELECCION", "global")
...
# ═══════════════════════════════════════════════════════════════════════════════
#  MOTORES DE IA  (Gemini 2.5 Pro en us-central1 + Gemini 3.1 Pro en endpoint global)
# ═══════════════════════════════════════════════════════════════════════════════
...
class MotoresIA:
    ...
    def __init__(self):
        ...
        # --- Cliente Gemini para la SELECCIÓN (endpoint GLOBAL) -------------------
        # Sustituye al antiguo cliente de Claude (Anthropic). El modelo de selección
        # gemini-3.1-pro-preview SOLO se sirve en el endpoint global de Vertex AI, por
        # lo que se crea un segundo cliente con location="global" (una llamada a este
        # modelo contra us-central1 devolvería 404). El cliente regional de arriba se
        # conserva intacto para el análisis multimodal y la búsqueda con grounding.
        self.gemini_seleccion = genai.Client(
            vertexai=True, project=self.project_id, location=VERTEX_LOCATION_SELECCION
        )
        log.ok(f"Gemini de selección «{AI_MODEL_SELECCION}» inicializado "
               f"(endpoint {VERTEX_LOCATION_SELECCION}; alternativa «{AI_MODEL_SELECCION_FALLBACK}»).")
...
- Sé exhaustivo con las características y especificaciones técnicas/eléctricas."""
    ...
    def buscar_mejor_opcion(self, necesidad: Necesidad, art_base: dict) -> Articulo:
        ...
        proveedores de confianza, y Gemini 3.1 Pro (endpoint global) para razonar y
        elegir la mejor opción (menor costo final, considerando logística si extranjero).
        """
  ...
  "candidatos": [
    {{
      ...
      "coincidencia": <0-100: % de coincidencia con los requisitos>
        ...
        # --- (b) Selección de la mejor opción con Gemini 3.1 Pro --------------------
...
{{
  ...
  "coincidencia": <0-100>
        ...
        # Razonamiento de selección con Gemini 3.1 Pro (cliente de endpoint global).
        # Si el proyecto aún no tiene habilitado gemini-3.1-pro-preview (p. ej. 404 por
        # estar en un canal restringido, o un 429 de cuota en preview), se recurre de
        # forma automática al modelo GA disponible (gemini-2.5-pro) en us-central1, de
        # modo que la selección NUNCA quede sin respuesta. El prompt y el parseo JSON
        # (_json_desde_texto) son idénticos sin importar qué modelo responda.
        cfg_sel = self._gtypes.GenerateContentConfig(max_output_tokens=8192)
        try:
            msg = self.gemini_seleccion.models.generate_content(
                model=AI_MODEL_SELECCION,
                contents=[self._gtypes.Part.from_text(text=prompt_seleccion)],
                config=cfg_sel,
            )
            texto = msg.text
        except Exception as exc:  # noqa: BLE001
            log.aviso(f"   · «{AI_MODEL_SELECCION}» no disponible ({exc}); se usa "
                      f"«{AI_MODEL_SELECCION_FALLBACK}» en {VERTEX_LOCATION_GEMINI}.")
            msg = self.gemini.models.generate_content(
                model=AI_MODEL_SELECCION_FALLBACK,
                contents=[self._gtypes.Part.from_text(text=prompt_seleccion)],
                config=cfg_sel,
            )
            texto = msg.text
