...
_FRAGMENTOS_RUTA_INVALIDA = [
    "/buscar", "/busqueda", "/búsqueda", "/search", "/404", "/not-found",
    "/page-not-found", "/error", "/cart", "/carrito", "/checkout",
]
_STOPWORDS = {
...
def _verificar_link_producto(url, nombre="", marca="", modelo="",
    ...
    if pf.path in ("", "/"):                       # redirigido a la portada → enlace incompleto
        return None
...
def _resolver_url_producto(res):
    ...
    for u in candidatos:
        ...
        if final:
            ...
            return final
    # 2) Verificación laxa: que al menos exista (200, sin 404/redirección a portada)
    for u in candidatos:
        ...
        if final:
            ...
            return final
    # 3) Nada verificable: conservar el propuesto
